...
Controls whether index segments should use the <<Compound File Segments>> format.
The default is `false`.

This applies to both newly written (flushed) segments and merged segments.
...
The maximum size (in megabytes) a segment may be and still use the <<Compound File Segments>> format when `useCompoundFile` is `true`; larger segments are written as non-compound.
The default is no limit.

[source,xml]
----
<maxCFSSegmentSizeMB>2048.0</maxCFSSegmentSizeMB>
----
...
[NOTE]
====
Whether segments use CFS is controlled by the <<usecompoundfile,`useCompoundFile`>> and <<maxcfssegmentsizemb,`maxCFSSegmentSizeMB`>> settings described above; both apply to newly written and merged segments alike.

The `noCFSRatio` and `maxCFSSegmentSizeMB` merge-policy options that earlier Lucene versions accepted were removed in Lucene 11 (compound-file selection is no longer a merge-policy concern); use the `<indexConfig>` settings above instead.
Such options remaining in an existing configuration are ignored with a warning rather than causing an error.
====
