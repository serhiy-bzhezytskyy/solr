...
public final class FieldTypePluginLoader extends AbstractPluginLoader<FieldType> {
  ...
  private Version parseConfiguredVersion(String configuredVersion, String pluginClassName) {
    ...
    if (!version.onOrAfter(Version.LUCENE_10_0_0)) {
      log.warn(
          "{} is using deprecated {}"
              + " emulation. You should at some point declare and reindex to at least 10.0, because "
              + "older emulation is deprecated and will be removed",
          pluginClassName,
          version);
    }
    return version;
